...
dpstyle: 'oswald: 3px;     color: #fff; background: #FF4500; box-shadow: 0 0 2px 0 rgba(0,0,0,0.14),0 2px 2px 0 rgba(0,0,0,0.12),0 1px 3px 0 rgba(0,0,0,0.2); border: none; border-color: #FF4500',
        ...
        },
   {
    ...
    function init() {
        // Проверяем, что мы на странице темы
    ...
    function addButtons() {
        ...
        if (!replyContainer) {
            setTimeout(addButtons, 500); // Повторяем попытку, если контейнер еще не загрузился
            return;
        }

        // Создаем кнопку для выбора ответов
    ...
    function compileTemplate(template) {
        // Простая замена шаблонных переменных
